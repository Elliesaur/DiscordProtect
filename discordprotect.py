...
def backup_file(file):
    ...
    new_file = "{}{}".format(file_name_inc_path + '_backup', file_ext)
...
def patch_asar(in_file):
    ...
    data = data.replace(user_data_path.encode('ansi'), user_data_path.replace(insecure_folder, secure_folder).encode('ansi'))
...
try:
    ...
    print('Could not rename folder "{}" to "{}" in "{}".\nPlease manually complete this step.'.format(insecure_folder, secure_folder, roaming))
